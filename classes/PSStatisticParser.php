...
class PSStatisticParser
{
    ...
    public function __construct($sid)
    {
        ...
        $this->oSurveyResponses = $this->_getAnswers($sid);
    ...
    public function createParsedDataBlockWithHook($hook = "")
    {
        ...
        if ($hook == "addRelatedSurveyResponses") {
            $aQuestionGroupList = $this->oSurvey->groups;
            $aHook = PSHooks::model()->findByAttributes(["sid" => $this->sid, "active" => 1]);
            $aHookData = [];
            ...
            if ($aHook) {
                $aHookData =  json_decode($aHook->hook_data);
                if ($aHookData) {
                    foreach ($aHookData as $data) {
                    }
                }
            }
    ...
    private function _getAnswers($sid, $additionalHook = false)
    {
        $result = [];
        $responses = SurveyDynamic::model($sid)->findAll();
    ...
    private function _getParserForType($type)
    {
        switch ($type) {
            ...
            case 'K':
            case 'Q':
                return "_parse_simple_subquestion_types";

            case 'M':
                return "_parse_multiple_choice";

            case 'P':
                return "_parse_multiple_choice_with_comments";

            case '1':
                return "_parse_array_multiscale";

            case ':':
            case ';':
                return "_parse_array_multi_flex";

            case '!':
            case 'L':
            case 'Z':
                return "_parse_list_dropdown";

            case 'O':
                return "_parse_list_with_comment";

            case 'R':
                return "_parse_ranking";

            case '|':
                return "_parse_fileupload";
    ...
    private function _applyResponseDataToQuestionData(&$aQuestionData)
    {
        foreach ($aQuestionData as $sSGQA => $aQuestionFieldset) {
            ...
            $aResponses = array_filter($aResponses, function ($v) {
                return !is_null($v);
            });
            $cleanResonses = array_filter($aResponses, function ($v) {
                return !is_null($v) && $v != "";
            });
            $aCalculations['count'] = count($aResponses);
            $aCalculations['countValid'] = count(array_filter($aResponses));
            $aCalculations['countInvalid'] = $aCalculations['count'] - $aCalculations['countValid'];
            $aCountValues = $this->count_array_values_with_nullables($cleanResonses);
            ...
            if ($this->isNumericallySafe($aQuestionFieldset['type'])) {
                ...
                $aCalculations['median'] = $this->calculate_median($cleanResonses);
                $aCalculations['average'] = $this->calculate_average($cleanResonses);
                $aCalculations['variance'] = $this->variance($cleanResonses);
                $aCalculations['std'] = $this->variance($cleanResonses, true);
            }
    ...
    public function count_array_values_with_nullables($arr)
    {
        ...
        foreach ($arr as $itrt) {
            ...
            if (!$itrt) {
                $result[gT('No answer')] = isset($result[gT('No answer')]) ? $result[gT('No answer')] + 1 : 0;
                continue;
            }
    ...
    private function getAnsweroptionBlock($oQuestion)
    {
        ...
        $aAnswers = array_map(function ($oA) {
            return $oA->attributes;
        }, $oQuestion->answers);
